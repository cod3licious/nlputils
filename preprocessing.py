...
from misc_utils.dict_utils import norm_dict, invert_dict2
...
def get_bigram_scores(text_words, min_bgfreq=2.):
    """
    given a dict of document word lists ({did:[w1,w2...,wn]}), compute unigram and bigram frequencies (of words) and give
    a score for every bigram. depending on this score (e.g. if higher than a threshold), bigram phrases can be identified
    in the raw texts before splitting them into words
    min_bgfreq says how often a bigram has to have to occur in the corpus to be recognized (if both words occur just once
    ...
    unigram_freq = {}
    bigram_freq = {}
    for did in text_words:
        for i, word in enumerate(text_words[did]):
            ...
            if i:
                try:
                    bigram_freq["%s %s"%(text_words[did][i-1],word)] += 1.
                except:
                    bigram_freq["%s %s"%(text_words[did][i-1],word)] = 1.
...
def find_bigrams(textdict, threshold=0.75):
    ...
    text_words = {did: textdict[did].split() for did in docids}
